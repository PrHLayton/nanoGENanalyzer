...
double calculate_cosThetaXStar(TLorentzVector lepton, TLorentzVector qSpec, TLorentzVector top)
{
   ...
   TVector3 qSpecUnit = qSpec.Vect().Unit();	          
   //TVector3 lightQ = qSpecUnit.Unit() * qSpec.Z();		//this method use the projection of the spectator quark in the Z direction in the laoratory frame


    TVector3 labZdir(0, 0, 1);			  //this method set a vector in the Z direction and the direction of the light quark is selected if the dot product between the spectator quark and Z
    TVector3 lightQ = labZdir;			  //is positive
    if (qSpecUnit.Dot(labZdir) < 0) lightQ = -labZdir;  
							  
							//the 2 methods give the same result
   lepton.Boost(InvariantTopBoost);
   qSpec.Boost(InvariantTopBoost);
